...
function ReleaseTransactions({onReplayTransaction}) {
  ...
  const handleReplayTransactionEvent = async (evt, txId, tokenAddress, amount) => {
    evt.preventDefault();
    setReleasing(true);
          ...
          <nav>
            <ul className="pagination justify-content-end">
              <li className={page > 0 ? "page-item" : "page-item disabled"}>
                <a className="page-link" href="/#" onClick={(e) => nextOrPrev(-1)}>Newer</a>
              </li>
              <li className={transactions.length === pageSize ? "page-item" : "page-item disabled"}>
                <a className="page-link" href="/#" onClick={(e) => nextOrPrev(1)}>Older</a>
              </li>
            </ul>
          </nav>
        : null
      ...
      { (transactions && transactions.length) || page !== 0
        ?
          <table className="table table-hover table-sm">
            <thead>
              <tr className="d-flex">
                <th className="col-4" scope="col">Block Number</th>
                <th className="col-3" scope="col">Token</th>
                <th className="col-4" scope="col">Quantity</th>
            ...
            <tbody>
              {transactions.map(tx => (
                <tr className="d-flex" key={tx.id}>
                  ...
                  <td className="col-1">
                    <Form noValidate onSubmit={(evt) => handleReplayTransactionEvent(evt, tx.id, tx.arguments.token.address, tx.arguments.amount)}>
                      <Button
                        type="submit"
