...
export default function TotalBalance(props) {
    ...
    const value = ((Number(children) * cDAIExchangeRate)/Number(BASE_FACTOR)).toFixed(6).toString()
    const number = value.substring(0, value.length - 4)
    const smallDecimals = value.substring(value.length - 4, value.length)
    return <>$ {numberWithCommas(number)}<span style={{color: "#958d9e", fontSize: "14px"}}>{smallDecimals}</span> USD</>
}
